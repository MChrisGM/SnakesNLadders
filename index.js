    ...
    function (socket) {
        ...
        socket.on('ready', function (data) {
          ...
          if (Object.keys(lobbies).length > 0 && lobbies[players[pID].lobby] ) {
                ...
                if(allReady && lobbies[players[pID].lobby].players.length >= 2){
                  io.in(players[pID].lobby).emit('start');
                  lobbies[key].settings.joinable = false;
                }
            }
        });
        ...
        socket.on('lobbyInfo', function (data) {
            ...
            if (Object.keys(lobbies).length > 0 && lobbies[players[pID].lobby] ) {
                for (let player of lobbies[players[pID].lobby].players) {
                    ...
                    playerInfo.ready.push(players[player].ready);
                }
...
function lobbyAvailable(lobbyid) {
    for (const [key, value] of Object.entries(lobbies)) {
        if (key == lobbyid) {
            if (lobbies[key].players.length < 10 && lobbies[key].settings.joinable) {
                return true;
            }
        }
    }
...
function joinLobby(pID) {
    ...
    var joined = false;
    for (const [key, value] of Object.entries(lobbies)) {
        if (lobbies[key].players.length < 10 && lobbies[key].settings.private != true && lobbies[key].settings.joinable) {
